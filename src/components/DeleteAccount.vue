...
const deleteAccount = async () => {
  ...
  try {
    ...
    if (response.success) {
      ...
      if (toastRef.value) {
        toastRef.value.addToast(response.message || 'Failed to delete account', 'error')
      }
    }
  } catch (error) {
      ...
      >
        ...
        <div v-if="accountInfo" class="mb-4 p-4 bg-gray-50 rounded-lg">
          ...
          <div class="text-sm text-gray-600">
            ...
            <p><strong>Member since:</strong> {{ new Date(accountInfo.created_at).toLocaleDateString() }}</p>
            <p><strong>Watchlist items:</strong> {{ accountInfo.watchlist_count }}</p>
          </div>
        </div>
        ...
        <div class="mb-4 p-4 bg-red-50 border border-red-200 rounded-lg">
          ...
          <p class="text-sm text-red-700">
            This action cannot be undone. All your data including watchlist, preferences, and account information will be permanently deleted.
          </p>
        </div>
